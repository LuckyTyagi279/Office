...
not_partition=0
not_format_fs=0
bootloader_file="u-boot-imx6q-sd.img"
...
if [ "${flash_images}" -eq "1" ]; then
    ...
    dd if=out/target/product/var_mx6/${bootloader_file} of=${node} bs=1k seek=69
    sync
    echo "boot image: ${bootimage_file}"
    sync
    dd if=out/target/product/var_mx6/${bootimage_file} of=${node}${part}1
    sync
    echo "recovery image: ${recoveryimage_file}"
    dd if=out/target/product/var_mx6/${recoveryimage_file} of=${node}${part}2
    sync
    echo "system image: ${systemimage_file}"
    dd if=out/target/product/var_mx6/${systemimage_file} of=${node}${part}5
    sync
    e2label ${node}${prefix}5 system
    sync
    dd if=device/variscite/common/firmware/SPL.mmc of=${node} bs=1k seek=1
    sync
    dd if=out/target/product/var_mx6/${bootloader_file} of=${node} bs=1k seek=69
