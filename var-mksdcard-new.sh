...
if [ "${flash_images}" -eq "1" ]; then
    ...
    sync
    echo "SPL: SPL.mmc"
    dd if=${CurrDir}/SPL.mmc of=${node} bs=1k seek=1
    sync
    echo "bootloader: ${bootloader_file}"
    dd if=${CurrDir}/${bootloader_file} of=${node} bs=1k seek=69
    sync
    echo "boot image: ${bootimage_file}"
    sync
    dd if=${CurrDir}/${bootimage_file} of=${node}${part}1
    sync
    echo "recovery image: ${recoveryimage_file}"
    dd if=${CurrDir}/${recoveryimage_file} of=${node}${part}2
    sync
    echo "system image: ${systemimage_file}"
    dd if=${CurrDir}/${systemimage_file} of=${node}${part}5
    sync
